                      ...
                      ,std::vector<esPolygon> &polygons)
{
    ...
    for(std::list<esConnectedComponent>::const_iterator c = components.begin(); c != components.end(); c++)
    {
        ...
        esPolygon newpoly;
        newpoly.pointSet.push_back(TilePosition(cx,cy));
                               ...
                               ,KERNEL::RectangleArray<esConnectedComponent*> &write_map)
{
    ...
    while (!q.empty())
    {
        ...
        {
			component->setTopLeft(Position(p.x(), p.y()));
        }
        int x = p.x();
        int y = p.y();
        ...
        if ( write_map[x][y] == NULL )
        {
            ...
            int max_x = std::min( (int)(x + 1),(int)(read_map.getWidth() - 1));
            int min_y = std::max( y - 1, 0);
            int max_y = std::min( (int)(y + 1),(int)(read_map.getHeight()-1));
    ...
    for (unsigned int x = 0; x < simplified_map.getWidth(); x++) {
        for (unsigned int y = 0; y < simplified_map.getHeight(); y++) {
            if (get_component[x][y] == NULL) {
				...
				current_component->setTopLeft(Position(x, y));
                floodFillwithComponent(simplified_map, Position(x, y), current_component, fill_type, get_component);
