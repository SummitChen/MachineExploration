...
TilePosition esPolygon::getCenter() const{
    double a = getArea();
    double cx = 0;
    double cy = 0;
    double temp;
    for (unsigned int i = 0, j = 1; i < this->pointSet.size(); i++, j++ ) {
        ...
        return TilePosition((int)cx, (int)cy);
    }
    
}

TilePosition esPolygon::getNearestPoint(const TilePosition & p) const
{
    double x3 = p.x();
    double y3 = p.y();
    TilePosition minp = TilePositions::Unknown;
...
bool esPolygon::isInside(const TilePosition & p) const{
    bool flag = false;
